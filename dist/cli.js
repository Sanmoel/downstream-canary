...
async function main() {
  const parsed = parseArguments(process4.argv.slice(2));
...
`);
  ...
  const config = await configFromInvocation(process4.cwd(), invocationValues(parsed.values));
...
`);
  process4.stdout.write(`Regressions: ${run.report.summary.candidateRegressions}
`);
...
try {
  ...
  process4.stderr.write(`downstream-canary: ${diagnosticExcerpt(errorDiagnostic(error))}
`);
  process4.exitCode = 2;
}
