...
function replaceLiteral(input, value) {
...
function redactSecrets(input, secretValues = secretValuesFromEnvironment()) {
  let output = input;
...
function truncateUtf8(input, maximumBytes) {
...
function diagnosticExcerpt(input) {
...
function parseConsumersInput(input) {
  const references = input.split(/\r?\n|,/).map((line) => line.trim()).filter((line) => line.length > 0 && !line.startsWith("#"));
...
function parseJsonCommandInput(input, label) {
...
async function configFromCliInvocation(cwd, values) {
...
function parseArguments(arguments_) {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    if (equals > 2) {
      const name2 = argument.slice(2, equals);
      const value2 = argument.slice(equals + 1);
      if (!name2 || !value2 || values.has(name2) || flags.has(name2)) {
        throw new CanaryError("configuration", "configuration", `Invalid or duplicate option --${name2}.`);
      }
...
function rejectUnknownOptions(parsed, allowedValues) {
...
// src/cli.ts
var HELP = `Downstream Canary ${VERSION}
...
function invocationValues(values) {
  return {
...
async function main() {
  const parsed = parseArguments(process5.argv.slice(2));
...
`);
  process5.stdout.write(`Regressions: ${run.report.summary.candidateRegressions}
`);
...
try {
  ...
  process5.stderr.write(`downstream-canary: ${diagnosticExcerpt(errorDiagnostic(error))}
`);
  process5.exitCode = 2;
}
