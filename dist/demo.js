...
function safeHostEnvironment(additions = {}) {
  const safePath = (process2.env.PATH ?? "/usr/local/bin:/usr/bin:/bin").split(delimiter).filter((component) => component.length > 0 && isAbsolute(component)).join(delimiter);
  const environment = {
...
async function readManifest(projectDirectory) {
  const manifestPath = join2(projectDirectory, "package.json");
...
async function validateLockfileSources(projectDirectory, lockfile) {
  const contents = await readFile(join2(projectDirectory, lockfile), "utf8");
...
async function validateProjectConfiguration(projectDirectory, manager) {
  ...
  for (const workspaceFile of ["pnpm-workspace.yaml", "pnpm-workspace.yml"]) {
    if (await metadataIfPresent(join2(projectDirectory, workspaceFile))) {
      throw new CanaryError(
        "unsupported-project",
        "configuration",
  ...
  if (manager === "yarn") {
    for (const unsupportedPath of [".yarn/cache", ".pnp.cjs", ".pnp.loader.mjs"]) {
      if (await metadataIfPresent(join2(projectDirectory, unsupportedPath))) {
        throw new CanaryError(
          "unsupported-project",
          "configuration",
    ...
    const yarnrcPath = join2(projectDirectory, ".yarnrc.yml");
...
async function detectPackageManager(projectDirectory, workingDirectory, overrides = {}, requireTest = true) {
  ...
  for (const [name2, file] of Object.entries(RECOGNIZED_LOCKFILES)) {
    if (await regularFileExists(join2(projectDirectory, file), file)) {
...
function lockfileGenerationCommand(manager) {
...
async function commitRepository(directory) {
  ...
  for (const command of [
    ...
    const result2 = await runProcess(command, { cwd: directory, environment, timeoutMs: 3e4 });
    if (result2.exitCode !== 0) throw new Error(result2.output);
  }
...
async function createFixtureWorld(dockerExecutable = "docker") {
  ...
  try {
    ...
    await import("node:fs/promises").then(
      ({ rm: rm4 }) => rm4(root, { recursive: true, force: true })
    );
    throw error;
  } finally {
...
// src/engine.ts
import { mkdtemp as mkdtemp4, rm as rm3 } from "node:fs/promises";
...
// src/consumer.ts
import { cp as cp2, lstat as lstat7, mkdir as mkdir5, mkdtemp as mkdtemp3, readFile as readFile4, realpath as realpath3, rm as rm2 } from "node:fs/promises";
import { join as join9, relative as relative4, resolve as resolve3, sep as sep5 } from "node:path";
...
async function applyDependencyPatch(projectDirectory, plan) {
  ...
  await writeFile2(manifestPath, applyEdits(source, edits), "utf8");
...
async function requireReservedPathsAbsent(project) {
  for (const reserved of [".downstream-canary", "node_modules"]) {
    try {
      await lstat7(join9(project, reserved));
    } catch (error) {
...
async function cleanNodeModules(project) {
  const nodeModules = join9(project, "node_modules");
...
async function runConsumer(consumer, artifact, docker, runtime, timeoutSeconds) {
  ...
  try {
    ...
    const candidateCheckout = join9(root, "candidate-checkout");
    ...
    const privateDirectory = join9(candidateProject, ".downstream-canary");
    await mkdir5(privateDirectory, { recursive: true });
    const candidateTarballPath = join9(privateDirectory, "candidate.tgz");
    ...
    const candidateManifest2 = await readManifest(candidateProject);
    ...
    const candidateLockPath = join9(candidateProject, candidateManager.lockfile);
...
async function writeReports(report, outputDirectory) {
  await mkdir6(outputDirectory, { recursive: true });
  ...
  const json = join10(outputDirectory, "downstream-canary-report.v1.json");
  const markdown = join10(outputDirectory, "downstream-canary-report.md");
  ...
  await writeFile3(json, stableStringify(report), "utf8");
  await writeFile3(markdown, markdownReport(report), "utf8");
...
async function runCandidateTests(candidateRoot, fixtureRoot, dockerExecutable) {
...
`);
  ...
  const candidate = mode === "compatible" ? world.candidateCompatible : world.candidateBreaking;
  const consumerId = mode === "regression" ? "npm-regression" : mode === "compatible" ? "npm-compatible" : "npm-preexisting";
  ...
  const run = await runCanary({
    ...
    outputDirectory: join12(world.root, `demo-output-${mode}`),
    timeoutSeconds: 180,
    dockerExecutable,
