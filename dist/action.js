...
// src/constants.ts
var PACKAGE_NAME = "downstream-canary";
var VERSION = "0.1.0";
var REPORT_SCHEMA_VERSION = "1.0.0";
var CONFIG_SCHEMA_VERSION = 1;
var NODE_VERSION = "24.19.0";
var RUNNER_IMAGE = "node:24.19.0-bookworm-slim@sha256:3638d9a6fe4030bd716be989438248074489337ba3275657f93595428be4fc03";
var DEFAULT_PACKAGE_MANAGER_VERSIONS = {
  npm: "11.17.0",
  pnpm: "11.21.0",
  yarn: "4.18.0"
};
...
// src/candidate.ts
import { cp, lstat as lstat3, mkdir, readdir, realpath } from "node:fs/promises";
import { basename as basename2, join as join2, relative, resolve, sep as sep2 } from "node:path";

// src/errors.ts
var CanaryError = class extends Error {
  kind;
  phase;
  diagnostic;
  constructor(kind, phase, message, options = {}) {
...
function asCanaryError(error, fallbackPhase) {
...
function resolveBlockMap({ composeNode: composeNode2, composeEmptyNode: composeEmptyNode2 }, ctx, bm, onError, tag) {
  ...
  for (const collItem of bm.items) {
    ...
    const valueProps = resolveProps(sep7 ?? [], {
    ...
    if (valueProps.found) {
      ...
      const valueNode = value ? composeNode2(ctx, value, valueProps, onError) : composeEmptyNode2(ctx, offset, sep7, null, valueProps, onError);
      if (ctx.schema.compat)
...
function resolveFlowCollection({ composeNode: composeNode2, composeEmptyNode: composeEmptyNode2 }, ctx, fc, onError, tag) {
  ...
  for (let i = 0; i < fc.items.length; ++i) {
    ...
    if (!isMap2 && !sep7 && !props.found) {
      const valueNode = value ? composeNode2(ctx, value, props, onError) : composeEmptyNode2(ctx, props.end, sep7, null, props, onError);
      ...
      const valueNode = value ? composeNode2(ctx, value, valueProps, onError) : valueProps.found ? composeEmptyNode2(ctx, valueProps.end, sep7, null, valueProps, onError) : null;
      if (valueNode) {
        if (isBlock(value))
          onError(valueNode.range, "BLOCK_IN_FLOW", blockMsg);
...
var GITHUB_COMPONENT = /^[A-Za-z0-9](?:[A-Za-z0-9_.-]{0,99})$/;
...
function validatePublicGitHubUrl(value) {
  let url;
  try {
    url = new URL(value);
  } catch (error) {
  ...
  if (components.length !== 2 || !components[0] || !components[1] || !GITHUB_COMPONENT.test(components[0]) || !GITHUB_COMPONENT.test(components[1])) {
    throw new CanaryError(
      "configuration",
      "configuration",
...
function validateSafePackageName(value) {
  if (value.length === 0 || value.length > 214 || value.includes("\0") || value.includes("\\") || isAbsolute(value)) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  const components = value.startsWith("@") ? value.slice(1).split("/") : value.split("/");
  const safeComponent = /^[a-z0-9][a-z0-9._~-]*$/;
  if (components.length !== (value.startsWith("@") ? 2 : 1) || components.some(
    (component) => component === "" || component === "." || component === ".." || !safeComponent.test(component)
  )) {
    throw new CanaryError(
      "configuration",
      "configuration",
...
function validateCommandArray(value, label) {
  ...
  const shellStringFlag = executable !== void 0 && (["sh", "bash", "dash", "ksh", "zsh"].includes(executable) && command.includes("-c") || ["cmd", "cmd.exe"].includes(executable) && command.some((part) => part.toLowerCase() === "/c") || ["powershell", "powershell.exe", "pwsh", "pwsh.exe"].includes(executable) && command.some(
    (part) => ["-command", "-encodedcommand"].includes(part.toLowerCase())
  ));
  if (shellStringFlag) {
    throw new CanaryError(
      "configuration",
...
function parsePackageManagerName(value, label) {
...
function parseProjectOverrides(value, label) {
  ...
  const result = {};
  if (value.packageManager !== void 0) {
    result.packageManager = parsePackageManagerName(
      value.packageManager,
      `${label}.packageManager`
    );
  }
  if (value.packageManagerVersion !== void 0) {
...
async function regularFileExists(path, label) {
...
function isPublicNpmRegistry(value) {
  ...
  try {
    ...
    return url.protocol === "https:" && url.hostname === "registry.npmjs.org" && url.port === "" && url.username === "" && url.password === "" && (url.pathname === "" || url.pathname === "/") && url.search === "" && url.hash === "";
  } catch {
    return false;
  }
}
async function validateLockfileSources(projectDirectory, lockfile) {
  ...
  for (const source of sources) {
    ...
    if (url.hostname !== "registry.npmjs.org" || url.port !== "" || url.username !== "" || url.password !== "" || url.search !== "" || url.hash !== "") {
      throw new CanaryError(
        "unsupported-project",
        "configuration",
...
function validateYarnRegistryConfiguration(value, path = ".yarnrc.yml") {
  if (Array.isArray(value)) {
    value.forEach(
      (item, index) => validateYarnRegistryConfiguration(item, `${path}[${index}]`)
    );
    return;
  }
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    if (key === "enableStrictSsl" && item === false || key === "checksumBehavior" && item !== "throw") {
      throw new CanaryError(
        "unsupported-project",
        "configuration",
    ...
    if (key === "cacheFolder" || key === "globalFolder" || key === "installStatePath" || key === "enableGlobalCache" && item !== true || key === "enableImmutableCache" && item === true) {
      throw new CanaryError(
        "unsupported-project",
        "configuration",
...
function managerCommands(name, version) {
...
async function validateProjectConfiguration(projectDirectory, manager) {
  ...
  if (await regularFileExists(npmrcPath, "Project .npmrc")) {
    ...
    for (const line of npmrc.split(/\r?\n/)) {
      ...
      if (["lockfile", "package-lock", "strict-ssl", "verify-store-integrity"].includes(
        normalizedKey
      ...
      if (["legacy-peer-deps", "force"].includes(normalizedKey) && normalizedSetting === "true") {
        throw new CanaryError(
          "unsupported-project",
          "configuration",
  ...
  if (manager === "yarn") {
    ...
    const yarnrc = parse(await readFile(yarnrcPath, "utf8"), {
      maxAliasCount: 0
    });
    ...
    if (yarnrc.yarnPath !== void 0 || yarnrc.plugins !== void 0) {
      throw new CanaryError(
        "unsupported-project",
        "configuration",
...
async function detectPackageManager(projectDirectory, workingDirectory, overrides = {}, requireTest = true) {
  const manifest = await readManifest(projectDirectory);
  if (manifest.workspaces !== void 0) {
    throw new CanaryError(
      "unsupported-project",
      "configuration",
  ...
  for (const [name2, file] of Object.entries(RECOGNIZED_LOCKFILES)) {
    if (await regularFileExists(join(projectDirectory, file), file)) {
      presentLockfiles.push({ name: name2, file });
    }
  }
  if (presentLockfiles.length !== 1) {
    throw new CanaryError(
      "configuration",
      "configuration",
      presentLockfiles.length === 0 ? "Exactly one recognized lockfile is required (package-lock.json, pnpm-lock.yaml, or yarn.lock)." : `Multiple package-manager lockfiles are ambiguous: ${presentLockfiles.map(({ file }) => file).join(", ")}.`
    );
  }
  const declared = manifest.packageManager ? parsePackageManagerDeclaration(manifest.packageManager) : void 0;
  ...
  const requestedVersion = validateExactVersion(
    overrides.packageManagerVersion ?? declared?.version ?? DEFAULT_PACKAGE_MANAGER_VERSIONS[name],
    `${name} version`
  );
...
async function sha256File(path) {
  return await new Promise((resolve5, reject) => {
    ...
    stream.once("end", () => resolve5(hash.digest("hex")));
  });
}
...
function parsePax(body) {
  ...
  while (offset < body.length) {
    ...
    if (!/^[1-9]\d*$/.test(lengthText) || !Number.isSafeInteger(length) || offset + length > body.length || space >= offset + length - 1 || body[offset + length - 1] !== 10) {
...
function validatePaxKeys(values) {
...
function validateMetadataPath(value) {
  if (hasUnsafeControl(value) || value.includes("\\") || posix.isAbsolute(value) || value.split("/").includes("..")) {
    throw new CanaryError(
      "tooling",
      "configuration",
...
function longHeaderField(body, label) {
  const zero = body.indexOf(0);
  const field = body.subarray(0, zero === -1 ? body.length : zero);
  try {
    return new TextDecoder("utf-8", { fatal: true }).decode(field).replace(/\n$/, "");
  } catch (error) {
    throw new CanaryError(
      "tooling",
      "configuration",
...
function safeEntryPath(value) {
  if (hasUnsafeControl(value) || value.includes("\\") || posix.isAbsolute(value)) {
...
function validateLink(entryPath, linkPath) {
  if (linkPath.length === 0 || hasUnsafeControl(linkPath) || linkPath.includes("\\") || posix.isAbsolute(linkPath)) {
    throw new CanaryError(
      "tooling",
      "configuration",
...
function parseTar(archive) {
  ...
  while (offset + 512 <= archive.length) {
    ...
    const mode = octalField(header, 100, 8) & 511;
    ...
    if (type === "x") {
      ...
      const linkPath = globalPax.get("linkpath") ?? nextPax.get("linkpath") ?? longLink ?? headerLink;
      entries.push({ path, type, linkPath, mode, body });
      nextPax = /* @__PURE__ */ new Map();
      longName = void 0;
      longLink = void 0;
    }
  ...
  if (!sawEnd || nextPax.size > 0 || longName !== void 0 || longLink !== void 0) {
    throw new CanaryError(
      "tooling",
      "configuration",
...
async function validateCandidateTarball(tarballPath, options) {
  ...
  const compressed = await readFile2(tarballPath);
  let archive;
  try {
  ...
  const packageFileHashes = /* @__PURE__ */ Object.create(null);
  const packageFileModes = /* @__PURE__ */ Object.create(null);
  const packageLinks = /* @__PURE__ */ Object.create(null);
  let packageJson;
...
var SECRET_NAME = /(TOKEN|KEY|SECRET|PASSWORD|CREDENTIAL|AUTH)/i;
var INLINE_SECRET = /\b(token|key|secret|password|credential|authorization|auth)\s*[:=]\s*([^\r\n,;]+)/gi;
var KNOWN_TOKEN = /\b(?:gh[pousr]_[A-Za-z0-9_]{20,}|github_pat_[A-Za-z0-9_]{20,}|npm_[A-Za-z0-9]{20,})\b/g;
function replaceLiteral(input2, value) {
  if (value.length < 4) return input2;
  return input2.split(value).join("[REDACTED]");
}
function sanitizeControls(input2) {
  let output = "";
  for (const character of stripVTControlCharacters(input2)) {
    const code = character.codePointAt(0) ?? 0;
    if (code <= 31 && character !== "\n" && character !== "\r" && character !== "	" || code === 127) {
...
function secretValuesFromEnvironment(environment = process.env) {
  return Object.entries(environment).filter(([name, value]) => SECRET_NAME.test(name) && Boolean(value)).map(([, value]) => value).filter((value) => value.length >= 4).sort((left, right) => right.length - left.length);
}
function redactSecrets(input2, secretValues = secretValuesFromEnvironment()) {
  let output = input2;
...
function truncateUtf8(input2, maximumBytes) {
  if (maximumBytes <= 0) return "";
  const bytes = Buffer.from(input2);
  if (bytes.byteLength <= maximumBytes) return input2;
  ...
  while (tailStart < bytes.byteLength && ((bytes[tailStart] ?? 0) & 192) === 128) {
    tailStart += 1;
  }
...
function diagnosticExcerpt(input2) {
  return truncateUtf8(redactSecrets(input2).trim(), MAX_DIAGNOSTIC_BYTES);
}
...
// src/candidate.ts
var COPY_EXCLUSIONS = /* @__PURE__ */ new Set([
  ".git",
  "node_modules",
  ".downstream-canary",
  ".downstream-canary-results"
]);
async function copyCandidateSource(source, destination) {
...
async function buildCandidate(config, docker, temporaryRoot, budget) {
  ...
  if (!rootMetadata.isDirectory() || rootMetadata.isSymbolicLink() || !projectMetadata.isDirectory() || projectMetadata.isSymbolicLink()) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  const checkout = join2(temporaryRoot, "candidate-source");
  await copyCandidateSource(sourceRoot, checkout);
  const project = resolve(checkout, workingDirectory);
  const cacheDirectory = join2(temporaryRoot, "candidate-cache");
  const outputDirectory = join2(cacheDirectory, "package-cache", "packed");
  ...
  const managerProvision = await docker.provisionManager(
    project,
    join2(temporaryRoot, "candidate-manager-provision"),
    manager,
  ...
  const buildCommand = config.buildCommand ?? (typeof copiedManifest.scripts?.build === "string" ? managerRunCommand(manager, "build") : void 0);
  if (buildCommand) {
  ...
  const npmProvision = manager.name === "npm" && manager.requestedVersion === DEFAULT_PACKAGE_MANAGER_VERSIONS.npm ? managerProvision : await docker.provisionManager(
    project,
    join2(temporaryRoot, "candidate-pack-manager-provision"),
    {
      name: "npm",
  ...
  const tarballPath = join2(outputDirectory, basename2(tarballs[0]));
...
// src/consumer.ts
import { cp as cp2, lstat as lstat7, mkdir as mkdir3, mkdtemp, readFile as readFile4, realpath as realpath3, rm } from "node:fs/promises";
import { join as join7, relative as relative4, resolve as resolve3, sep as sep5 } from "node:path";
...
import { delimiter, isAbsolute as isAbsolute2 } from "node:path";
function safeHostEnvironment(additions = {}) {
  const safePath = (process2.env.PATH ?? "/usr/local/bin:/usr/bin:/bin").split(delimiter).filter((component) => component.length > 0 && isAbsolute2(component)).join(delimiter);
  const environment = {
    PATH: safePath || "/usr/local/bin:/usr/bin:/bin",
    LANG: process2.env.LANG ?? "C.UTF-8",
    LC_ALL: process2.env.LC_ALL ?? "C.UTF-8",
    ...additions
  };
  if (process2.env.SystemRoot) environment.SystemRoot = process2.env.SystemRoot;
  return environment;
}
function killProcessTree(pid, signal) {
  if (!Number.isInteger(pid) || pid <= 0) return;
  try {
    if (process2.platform === "win32") process2.kill(pid, signal);
    else process2.kill(-pid, signal);
  } catch {
  }
}
async function runProcess(command, options = {}) {
  ...
  const maximumOutputBytes = options.maximumOutputBytes ?? MAX_PROCESS_OUTPUT_BYTES;
  const secretValues = [
    .../* @__PURE__ */ new Set([
...
async function git(args, cwd, configDirectory, timeoutMs) {
  const command = ["git", "-c", "credential.helper=", ...args];
...
async function checkoutConsumer(consumer, destination, configDirectory, budget) {
  ...
  await mkdir2(configDirectory, { recursive: true });
  await git(
  ...
  try {
    await lstat4(join3(destination, ".gitmodules"));
  } catch (error) {
...
// src/classifier.ts
function classifyCompatibility(baselinePassed, candidatePassed) {
  if (baselinePassed) {
    return candidatePassed ? "compatible" : "candidate-regression";
  }
  return candidatePassed ? "candidate-improvement" : "inconclusive-preexisting";
}
...
function parseTree(text, errors = [], options = ParseOptions.DEFAULT) {
  ...
  const visitor = {
    ...
    onSeparator: (sep7, offset, length) => {
      if (currentParent.type === "property") {
        if (sep7 === ":") {
...
function visit3(text, visitor, options = ParseOptions.DEFAULT) {
...
function dependencyValue(manifest, field, packageName) {
  const section = manifest[field];
  const value = section?.[packageName];
  return typeof value === "string" ? value : void 0;
}
function validateSupportedSpecifier(specifier) {
  ...
  if (specifier.startsWith("file:")) {
    ...
    if (!target.toLowerCase().endsWith(".tgz") || target.startsWith("/") || target.startsWith("~") || target.includes("\\") || target.split("/").includes("..")) {
      throw new CanaryError(
        "unsupported-project",
        "candidate-injection",
  ...
  if (specifier.length === 0 || /^[a-z][a-z0-9+.-]*:/i.test(specifier) || specifier.startsWith(".") || specifier.startsWith("/") || specifier.includes("\\") || specifier.includes("/")) {
    throw new CanaryError(
      "unsupported-project",
      "candidate-injection",
...
function planDependencyPatch(manifest, packageName, newSpecifier) {
  ...
  if (matches.length === 0) {
    ...
    throw new CanaryError(
      "unsupported-project",
      "candidate-injection",
      peer ? "Peer-only dependencies are unsupported in v0.1." : "The candidate is not a root direct dependency; transitive-only dependencies are unsupported."
    );
  }
  if (matches.length > 1) {
    throw new CanaryError(
...
async function applyDependencyPatch(projectDirectory, plan) {
  const manifestPath = join4(projectDirectory, "package.json");
...
async function verifyDependencyPatch(projectDirectory, plan) {
  const manifest = await readManifest(projectDirectory);
  ...
  for (const field of DIRECT_FIELDS) {
    if (field !== plan.field && manifest[field]?.[plan.packageName] !== void 0) {
      throw new CanaryError(
        "tooling",
        "candidate-injection",
...
import { dirname, join as join5, relative as relative2, resolve as resolve2, sep as sep3 } from "node:path";
function packagePath(projectDirectory, packageName) {
  return join5(projectDirectory, "node_modules", ...packageName.split("/"));
}
...
async function verifyInstalledCandidate(projectDirectory, artifact) {
  ...
  let realProjectRoot;
  try {
    realProjectRoot = await realpath2(projectDirectory);
    installedRoot = await realpath2(logicalRoot);
  } catch (error) {
    throw new CanaryError(
      "tooling",
  ...
  for (const [path, expectedHash] of Object.entries(artifact.packageFileHashes)) {
    const installedPath = join5(installedRoot, ...path.split("/"));
    ...
    if (expectedMode === void 0 || (metadata.mode & 511) !== expectedMode) {
      throw new CanaryError(
        "tooling",
        "candidate-verification",
  ...
  for (const [path, expectedTarget] of Object.entries(artifact.packageLinks)) {
    ...
    const resolvedTarget = resolve2(dirname(installedPath), expectedTarget);
...
import { join as join6, relative as relative3, sep as sep4 } from "node:path";
...
async function validateLaneOutputs(options) {
  ...
  for (const path of generatedNames) {
    if (options.protectedPaths.has(path)) continue;
    const metadata = await lstat6(join6(options.root, ...path.split("/")));
    if (!metadata.isFile() || metadata.isSymbolicLink()) {
      throw new CanaryError(
        "unsupported-project",
        options.phase,
...
function isWithin2(parent, child) {
  const path = relative4(resolve3(parent), resolve3(child));
  return path === "" || !path.startsWith(`..${sep5}`) && path !== "..";
}
async function projectPath(checkout, workingDirectory) {
  if (workingDirectory !== ".") {
    throw new CanaryError(
      "unsupported-project",
      "configuration",
  ...
  const path = resolve3(checkout, workingDirectory);
  if (!isWithin2(checkout, path)) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  const metadata = await lstat7(path).catch((error) => {
    throw new CanaryError(
      "unsupported-project",
      "configuration",
  ...
  const realCheckout = await realpath3(checkout);
  const realProject = await realpath3(path);
  if (!isWithin2(realCheckout, realProject)) {
    throw new CanaryError(
      "unsupported-project",
      "configuration",
...
async function requireReservedPathsAbsent(project) {
  for (const reserved of [".downstream-canary", "node_modules"]) {
    try {
      await lstat7(join7(project, reserved));
    } catch (error) {
...
function requireNoUnexpectedChanges(before, after, expectedChanged, message) {
  const diff = diffSnapshots(before, after);
  const unexpected = [
    ...diff.added.map((path) => `added:${path}`),
    ...diff.removed.map((path) => `removed:${path}`),
    ...diff.changed.filter((path) => !expectedChanged.has(path)).map((path) => `changed:${path}`)
  ];
...
function testPhase(install, test) {
  return {
    ...
    testStatus: test === void 0 ? "not-run" : test.exitCode === 0 && !test.timedOut ? "pass" : "fail",
    durationMs: install.durationMs + (test?.durationMs ?? 0)
  };
}
function appendDiagnostic(state, label, result) {
  const content = result.output.trim();
  if (content) state.diagnostic += `${state.diagnostic ? "\n\n" : ""}[${label}]
${content}`;
}
function buildResult(consumer, artifact, runtime, state, classification, failurePhase, durationMs) {
  return {
...
async function cleanNodeModules(project) {
  const nodeModules = join7(project, "node_modules");
  const resolved = resolve3(nodeModules);
  if (!isWithin2(project, resolved) || resolved === resolve3(project)) {
    throw new CanaryError(
      "tooling",
      "candidate-injection",
...
async function runConsumer(consumer, artifact, docker, runtime, budget) {
  const started = performance.now();
  const root = await mkdtemp(join7(tmpdir(), "downstream-canary-consumer-"));
  const state = {
    manager: null,
    baseline: NOT_RUN,
    candidate: NOT_RUN,
  ...
  try {
    const baselineCheckout = join7(root, "baseline-checkout");
    const candidateCheckout = join7(root, "candidate-checkout");
    await checkoutConsumer(
      consumer,
      baselineCheckout,
      join7(root, "git-config-baseline"),
      budget
    );
    await checkoutConsumer(
      consumer,
      candidateCheckout,
      join7(root, "git-config-candidate"),
      budget
    );
    ...
    const originalLockPath = join7(baselineProject, manager.lockfile);
    state.originalLockfileHash = await sha256File(originalLockPath);
    const managerProvision = await docker.provisionManager(
      baselineProject,
      join7(root, "manager-provision"),
      manager,
    ...
    const baselineCache = join7(root, "baseline-cache");
    ...
    const privateDirectory = join7(candidateProject, ".downstream-canary");
    await mkdir3(privateDirectory, { recursive: true });
    const candidateTarballPath = join7(privateDirectory, "candidate.tgz");
    await cp2(artifact.tarballPath, candidateTarballPath, {
      force: false,
      errorOnExist: true
    });
    ...
    requireNoUnexpectedChanges(
      candidateOriginal,
      afterPatch,
      /* @__PURE__ */ new Set(["package.json"]),
      "Candidate manifest patch was not mechanically isolated."
    );
    const candidateCache = join7(root, "candidate-cache");
    ...
    requireNoUnexpectedChanges(
      afterPatch,
      afterLock,
      /* @__PURE__ */ new Set([candidateManager.lockfile]),
      "Candidate lockfile generation changed files outside the expected lockfile."
    );
    const candidateLockPath = join7(candidateProject, candidateManager.lockfile);
    const candidateLockMetadata = await lstat7(candidateLockPath).catch(
      (error) => {
        throw new CanaryError(
          "tooling",
          "candidate-injection",
    ...
    const lockText = await readFile4(candidateLockPath, "utf8");
    ...
    const failurePhase = classification === "candidate-regression" ? "candidate-test" : classification === "inconclusive-preexisting" || classification === "candidate-improvement" ? "baseline-test" : null;
    return buildResult(
      consumer,
      artifact,
      runtime,
...
import { mkdir as mkdir4, mkdtemp as mkdtemp2, rm as rm2 } from "node:fs/promises";
...
function validateLocalDockerHost(value) {
  ...
  if (url.protocol !== "unix:" || url.hostname !== "" || !isAbsolute3(decodeURIComponent(url.pathname)) || url.search !== "" || url.hash !== "" || url.username !== "" || url.password !== "") {
    throw new CanaryError(
      "configuration",
      "docker",
...
function enforceTrustedActionEnvironment(environment = process.env) {
  ...
  if (!workspace || !isAbsolute3(workspace)) {
    throw new CanaryError(
      "configuration",
      "configuration",
...
var DockerRunner = class {
  ...
  async #environment() {
    this.#configDirectory ??= await mkdtemp2(join8(tmpdir2(), "downstream-canary-docker-"));
    return safeHostEnvironment({
  ...
  async provisionManager(workspace, cacheDirectory, manager, timeoutSeconds, budget) {
    this.#assertOpen();
    const corepackDirectory = join8(cacheDirectory, "corepack");
    await mkdir4(corepackDirectory, { recursive: true });
  ...
  async run(options) {
    this.#assertOpen();
    const cacheDirectory = options.cacheDirectory ?? join8(options.workspace, ".downstream-canary", "runtime-cache");
    const corepackDirectory = options.managerProvision?.corepackDirectory ?? join8(cacheDirectory, "empty-corepack");
    await mkdir4(corepackDirectory, { recursive: true });
  ...
  async #run(options, corepackDirectory, corepackReadOnly) {
    const cacheDirectory = options.cacheDirectory ?? join8(options.workspace, ".downstream-canary", "runtime-cache");
    const packageCacheDirectory = join8(cacheDirectory, "package-cache");
    await mkdir4(packageCacheDirectory, { recursive: true });
  ...
  async #dispose() {
    ...
    if (this.#configDirectory) {
      await rm2(this.#configDirectory, { recursive: true, force: true });
...
// src/report.ts
import { lstat as lstat8, mkdir as mkdir5, writeFile as writeFile2 } from "node:fs/promises";
...
function terminalTable(report) {
  const rows = report.results.map((result) => ({
    consumer: result.repositoryUrl.replace("https://github.com/", ""),
    manager: result.packageManager && result.actualPackageManagerVersion ? `${result.packageManager}@${result.actualPackageManagerVersion}` : "n/a",
  ...
  const widths = headers.map(
    (header, index) => Math.max(header.length, ...values.map((row) => row[index]?.length ?? 0))
  );
  const format2 = (row) => row.map((value, index) => value.padEnd(widths[index] ?? value.length)).join("  ");
  return [
    format2(headers),
    format2(widths.map((width) => "-".repeat(width))),
    ...values.map(format2)
  ].join("\n");
}
  ...
  const lines = [
    ...
    ...report.results.map(
      (result) => [
        ...
        `\`${result.commit}\``,
        result.packageManager ? `${result.packageManager}@${result.actualPackageManagerVersion ?? result.declaredPackageManagerVersion ?? "unknown"}` : "n/a",
...
async function writeReports(report, outputDirectory) {
  ...
  const outputMetadata = await lstat8(outputDirectory);
  if (!outputMetadata.isDirectory() || outputMetadata.isSymbolicLink()) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  const json = join9(outputDirectory, "downstream-canary-report.v1.json");
  const markdown = join9(outputDirectory, "downstream-canary-report.md");
  for (const target of [json, markdown]) {
    try {
      const metadata = await lstat8(target);
      if (!metadata.isFile() || metadata.isSymbolicLink()) {
        throw new CanaryError(
          "configuration",
          "configuration",
  ...
  await writeFile2(json, stableStringify(report), "utf8");
  await writeFile2(markdown, markdownReport(report), "utf8");
...
async function runCanary(config) {
  ...
  const temporaryRoot = await mkdtemp3(join10(tmpdir3(), "downstream-canary-run-"));
  ...
  try {
    ...
    const runtime = await docker.runtimeInfo(
      join10(temporaryRoot, "candidate-source", config.candidate.workingDirectory),
...
// src/config.ts
import { lstat as lstat9, readFile as readFile5 } from "node:fs/promises";
import { dirname as dirname2, join as join11, resolve as resolve4, sep as sep6 } from "node:path";
async function regularFileExists2(path, label) {
  try {
...
async function assertSafeDirectoryPath(root, relativePath, label) {
  if (relativePath === ".") return;
  let current = resolve4(root);
  for (const component of relativePath.split(sep6)) {
...
function parseConsumerReference(reference) {
  ...
  const repositoryUrl = repository.startsWith("https://") ? validatePublicGitHubUrl(repository) : validatePublicGitHubUrl(`https://github.com/${repository}`);
  return { repositoryUrl, commit, workingDirectory: "." };
}
function parseConsumersInput(input2) {
  const references = input2.split(/\r?\n|,/).map((line) => line.trim()).filter((line) => line.length > 0 && !line.startsWith("#"));
...
function parseConsumerObject(value, defaults, index) {
  ...
  const repositoryUrl = value.repository.startsWith("https://") ? validatePublicGitHubUrl(value.repository) : validatePublicGitHubUrl(`https://github.com/${value.repository}`);
  ...
  if (value.workingDirectory !== void 0 && typeof value.workingDirectory !== "string") {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  const workingDirectory = typeof value.workingDirectory === "string" ? validateRelativeWorkingDirectory(value.workingDirectory) : ".";
  if (workingDirectory !== ".") {
    throw new CanaryError(
      "unsupported-project",
      "configuration",
...
function parseRawConfig(value, candidateRoot) {
  if (!isPlainObject(value)) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  rejectUnknownKeys(
    value,
    /* @__PURE__ */ new Set(["version", "candidate", "defaults", "consumers", "outputDirectory", "timeoutSeconds", "runTimeoutSeconds"]),
    "configuration"
  );
  ...
  if (candidateValue !== void 0 && !isPlainObject(candidateValue)) {
  ...
  if (candidateWorkingDirectory !== void 0 && typeof candidateWorkingDirectory !== "string") {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  if (value.runTimeoutSeconds !== void 0 && (!Number.isInteger(value.runTimeoutSeconds) || value.runTimeoutSeconds < MIN_RUN_TIMEOUT_SECONDS || value.runTimeoutSeconds > MAX_RUN_TIMEOUT_SECONDS)) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  const normalizedCandidateWorkingDirectory = typeof candidateWorkingDirectory === "string" ? validateRelativeWorkingDirectory(candidateWorkingDirectory) : ".";
  ...
  if (value.consumers !== void 0 && !Array.isArray(value.consumers)) {
    throw new CanaryError("configuration", "configuration", "consumers must be a list.");
  }
  const consumers = (value.consumers ?? []).map(
    (consumer, index) => parseConsumerObject(consumer, defaults, index)
  );
  if (value.outputDirectory !== void 0 && (typeof value.outputDirectory !== "string" || value.outputDirectory.length === 0)) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  if (value.timeoutSeconds !== void 0 && (!Number.isInteger(value.timeoutSeconds) || value.timeoutSeconds < MIN_TIMEOUT_SECONDS || value.timeoutSeconds > MAX_TIMEOUT_SECONDS)) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  return {
    ...
    consumers,
    defaults,
    ...typeof value.outputDirectory === "string" ? { outputDirectory: value.outputDirectory } : {},
    ...typeof value.timeoutSeconds === "number" ? { timeoutSeconds: value.timeoutSeconds } : {},
    ...typeof value.runTimeoutSeconds === "number" ? { runTimeoutSeconds: value.runTimeoutSeconds } : {}
  };
}
function validateConsumerCount(consumers) {
...
async function resolveRunConfig(options) {
  ...
  const candidateRoot = resolve4(options.cwd, candidateRootRelative);
  const configurationSource = options.configurationSource ?? "cli";
  if (configurationSource === "none" && options.configPath !== void 0) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  if (configurationSource === "cli") {
    ...
    await assertSafeDirectoryPath(
      options.cwd,
      dirname2(configPathRelative),
      "Configuration parent directory"
    );
    const configPath = resolve4(options.cwd, configPathRelative);
    const hasConfig = await regularFileExists2(configPath, "Configuration file");
    ...
    if (hasConfig) {
      raw = parseRawConfig(
        parse(await readFile5(configPath, "utf8"), { maxAliasCount: 0 }),
        candidateRoot
      );
    }
  }
  const inputConsumers = options.consumersText ? parseConsumersInput(options.consumersText).map(
    (consumer) => mergeOverrides(consumer, raw.defaults, options.consumerOverrides)
  ) : raw.consumers.map(
    (consumer) => mergeOverrides(consumer, options.consumerOverrides)
  );
  validateConsumerCount(inputConsumers);
  const timeoutSeconds = options.timeoutSeconds ?? raw.timeoutSeconds ?? DEFAULT_TIMEOUT_SECONDS;
  ...
  const runTimeoutSeconds = options.runTimeoutSeconds ?? raw.runTimeoutSeconds ?? DEFAULT_RUN_TIMEOUT_SECONDS;
  if (!Number.isInteger(runTimeoutSeconds) || runTimeoutSeconds < MIN_RUN_TIMEOUT_SECONDS || runTimeoutSeconds > MAX_RUN_TIMEOUT_SECONDS) {
    throw new CanaryError(
      "configuration",
      "configuration",
  ...
  return {
    ...
    outputDirectory: resolve4(options.cwd, outputDirectoryRelative),
    timeoutSeconds,
    runTimeoutSeconds,
...
function parseJsonCommandInput(input2, label) {
...
// src/invocation.ts
function optionalManager(value, label) {
  return value === void 0 ? void 0 : parsePackageManagerName(value, label);
}
function optionalVersion(value, label) {
  return value === void 0 ? void 0 : validateExactVersion(value, label);
}
...
function candidateOverrides(values) {
...
function consumerOverrides(values) {
  return withoutUndefined({
    packageManager: optionalManager(
...
async function configFromInvocation(cwd, values, configurationSource) {
  let timeoutSeconds;
  let runTimeoutSeconds;
  if (values.timeoutSeconds !== void 0) {
  ...
  if (values.runTimeoutSeconds !== void 0) {
  ...
  return await resolveRunConfig({
    ...
    ...configurationSource === "cli" && values.config ? { configPath: values.config } : {},
...
async function configFromActionInvocation(cwd, values) {
...
// src/action.ts
function input(name) {
  const value = process5.env[`INPUT_${name.toUpperCase()}`]?.trim();
...
async function setOutput(name, value) {
  const outputPath = process5.env.GITHUB_OUTPUT;
  if (!outputPath) return;
  const delimiter2 = `DOWNSTREAM_CANARY_${randomUUID2()}`;
  await appendFile(
    outputPath,
    `${name}<<${delimiter2}
${value}
...
`);
  ...
  if (process5.env.GITHUB_STEP_SUMMARY) {
    const markdown = await import("node:fs/promises").then(
      ({ readFile: readFile6 }) => readFile6(run.paths.markdown, "utf8")
    );
    await appendFile(process5.env.GITHUB_STEP_SUMMARY, markdown, "utf8");
  }
...
try {
  ...
  process5.stderr.write(`::error::${escapeWorkflowCommand(message)}
`);
  process5.exitCode = 2;
}
